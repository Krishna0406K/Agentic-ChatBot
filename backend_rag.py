...
if not groq_key:
    raise ValueError("GROQ_API_KEY not found in environment variables. Please get a free key from https://console.groq.com/keys")
...
def ingest_pdf(file_bytes: bytes, thread_id: str, filename: Optional[str] = None) -> dict:
    ...
    try:
        ...
        for doc in chunks:
            # Get the page content and ensure it's a string
            ...
            # Only add if non-empty after stripping
            if text:
                texts.append(text)
        ...
        _THREAD_METADATA[str(thread_id)] = {
            ...
            "documents": len(docs),
            "chunks": len(texts),  # Use len(texts) instead of len(chunks)
        }
